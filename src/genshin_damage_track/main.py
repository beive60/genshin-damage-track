...
def extract(
    ...
    if plot or plot_output is not None:
        plot_damage(result, output_path=plot_output, show=plot)
...
def plot(
    ...
    plot: Annotated[bool, typer.Option("--plot", "-p", help="Show the graph interactively.")] = False,
    plot_output: Annotated[
        Optional[Path],
        typer.Option("--plot-output", help="Save the graph to this file instead of showing it."),
    ] = None,
    ...
    show = plot or plot_output is None
    plot_damage(result, output_path=plot_output, show=show)
    if plot_output is not None:
        typer.echo(f"Graph saved to {plot_output}")
